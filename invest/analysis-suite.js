...
async function renderCompany(r){
  ...
  ws.innerHTML=
    ...
    '<div class="insight-grid"><article class="insight-card"><h3>O que mudou?</h3>'+changesHtml(payload,data)+'</article><article class="insight-card"><h3>Cotação recente</h3>'+priceSpark(payload?.price_history||[])+'</article></div>'+
    '<div class="method-note"><strong>Data-base</strong><p>Fundamentos: '+esc(data.fundamentals_reference_date||'não informada')+' • Cotação: '+esc(data.price_quoted_at?new Date(data.price_quoted_at).toLocaleString('pt-BR'):'não informada')+'. Os dois conjuntos podem ter periodicidades diferentes.</p></div>'
