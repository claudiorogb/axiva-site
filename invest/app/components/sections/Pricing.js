...
export default function Pricing() {
    return (React.createElement("section", { id: "planos", className: "bg-white py-16 lg:py-20" },
        React.createElement("div", { className: "mx-auto max-w-[1700px] px-4 lg:px-8" },
            ...
            React.createElement("div", { className: "mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4" }, plans.map((plan) => (React.createElement("article", { key: plan.name, className: "relative flex min-h-[465px] flex-col overflow-hidden rounded-[28px] border border-[#21b8b7] bg-[#08243f] p-8 text-white" },
                ...
                React.createElement("a", { href: plan.href, target: "_blank", rel: "noopener noreferrer", className: "mt-auto inline-flex min-h-16 items-center justify-center rounded-2xl bg-gradient-to-r from-[#2cbab9] to-[#61d5d3] px-4 text-center text-sm font-extrabold text-black transition-opacity hover:opacity-90 sm:text-base" }, plan.button))))))));
}
